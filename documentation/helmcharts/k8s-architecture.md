...
The cluster is divided into three namespaces, each grouping related concerns:
...
│  │ • Workers (×2)    │  │                                    │  │
│  │ • Triggerer       │  │ (internal sub-services:)           │  │
│  │ • [PostgreSQL]    │  │                                    │  │
│  │ • [Redis]         │  │                                    │  │
│  └───────────────────┘  └────────────────────────────────────┘  │
│                                                                 │
...
| `datalake` | Persistent data store and database management UI |
...
All cross-namespace communication uses Kubernetes in-cluster DNS (`<service>.<namespace>.svc.cluster.local`). No Ingress or external load balancer is configured; access from the developer machine uses `kubectl port-forward`.
...
## Developer Access (Port Forwarding)

No Ingress controller is deployed. Access to all UIs is via `kubectl port-forward`:
...
The script: validates `.env`, creates all namespaces and secrets, adds Helm repos, installs all five releases in dependency order, applies `ServiceMonitors`, and waits for each critical pod to be ready.
